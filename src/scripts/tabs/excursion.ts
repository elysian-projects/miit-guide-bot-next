...
export const initUserForExcursion = (ctx: Context, storeController: StoreController, options: {userId: UserId, location: keyof LocationList}) => {
  const {location, userId} = options;
